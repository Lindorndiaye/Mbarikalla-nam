    ...
    public function addAction(Request $request)
    {
        if (!$request->isXmlHttpRequest()) {
            if ($request->isMethod('POST') && $_POST['form'] == 'contrat') {
                ...
                $bienr = $this->getDoctrine()
                 ->getManager()
                 ->getRepository('KEURGUIimmoBundle:bien')
                 ->find($res->getBien()->getId());
                $bienr->setEtat(1);
                $em->persist($bienr);
                $em->flush();
                $em = $this->getDoctrine()->getEntityManager();
                $reservation = $em->getRepository('KEURGUIimmoBundle:reservation')
                                    ->findBy(array('bien' => $res->getBien()->getId(), 'etat' => 0));
                foreach ($reservation as $resrvation) {
